...
references_dict = {"focus": ("Направленность обучения", (['XXX', 10], ['Направленность кратко', 140], ['Полное наименование', 350])),
                   "mode": ("Режим пребывания", (['XXX', 10], ['Режим кратко', 140], ['Полное наименование', 350])),
                   "benefit": ("Льгота по направлению", (['XXX', 10], ['Льгота кратко', 140], ['Полное наименование', 350])),
                   "citizenship": ("Гражданство", (['XXX', 10], ['Гражданство кратко', 140], ['Полное наименование', 350])),
                   }

print(references_dict["benefit"])
print(references_dict["benefit"][0])
print(references_dict["benefit"][1])



"""SELECT tbl_2.ФИО_ребенка, tbl_2.комп_2024, tbl_2.комп_2024_номер, tbl_2.ставка_комп_24, tbl_2.журнал_нр, 
tbl_2.Название_группы, Val([комп_2024_номер]) AS Numder
FROM tbl_2
WHERE (((tbl_2.комп_2024)=True) AND ((tbl_2.Приказ_отчисление_дата) Is Null) AND (((val(tbl_2.журнал_нр)=1) AND 
((val(tbl_2.комп_2024_номер)>216) OR val(tbl_2.ставка_комп_24)=100)) OR ((val(tbl_2.журнал_нр)=2) AND 
((val(tbl_2.комп_2024_номер)>259) OR val(tbl_2.ставка_комп_24)=100))));"""
